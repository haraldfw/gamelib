...
public class AStar {
     ...
     * @param heuristic The method of estimating the best next node. Manhattan for performance.
     *                  Pythagorean for accuracy.
     */
    ...
    public void defineWorldFromPolyedge(UnifiablePolyedge polyedge) {
        ...
        for (Line l : polyedge.getEdges()) {
            if (getNodeAt(l.x, l.y, nodes) == null && !(nrLinesnPoint(l.x, l.y, polyedge.getEdges())
                    > 2)) {
                nodes.add(new StarNode(l.x, l.y));
            }
            if (getNodeAt(l.x2, l.y2, nodes) == null && !(nrLinesnPoint(l.x2, l.y2, polyedge.getEdges())
                    > 2)) {
                nodes.add(new StarNode(l.x2, l.y2));
            }
        }
        ...
        for (StarNode n : nodes) {
            ...
            Vector2
                    v1 =
                    (n.x == l1.x && n.y == l1.y) ? new Vector2(l1.x2, l1.y2) : new Vector2(l1.x, l1.y);
            Vector2
                    v2 =
                    (n.x == l2.x && n.y == l2.y) ? new Vector2(l2.x2, l2.y2) : new Vector2(l2.x, l2.y);
            // It works. Don't question it
            Vector2
                    lIn =
                    new Vector2(new Vector2(v2).sub(n.x, n.y).add(new Vector2(v1).sub(n.x, n.y)).scl(0.1f)
                            .add(n.x, n.y));
            if (Intersect
                    .pointInsidePolyedge(lIn.x, lIn.y, polyedge.getEdges(), polyedge.getOrigin().x - 10)) {
                toRemove.add(n);
            }
        }
        ...
        System.out
                .println("Done purging inner corner nodes. New size of nodeArray is: " + nodes.size());
    ...
    public void connectNodesBySight(ArrayList<Line> intersectors) {
        ...
        for (StarNode n : nodes) {
            for (StarNode s : nodes) {
                ...
                if (!Intersect
                        .pointInsidePolyedge(l.x + (l.x2 - l.x) / 2, l.y + (l.y2 - l.y) / 2, intersectors,
                                -10)) {
                    continue;
                }
            ...
            for (Connection c : s.getConnections()) { // For every s-node connection
                ...
                for (StarNode s2 : nodes) { // For every node in nodeList. StarNode s2
                    ...
                    for (Connection c2 : s2.getConnections()) { // for every s2-node connection
                        ...
                        if (cl.equalTo(c2l)) {
                            continue; // If parallel, no intersection; skip
                        }
    ...
    /**
     * Returns node at given x and y coordinates if such a node exists. Will return null if no node
     * exists with given coordinates
     *
    ...
    private class PathfindingList {
        ...
        public StarNode getSmallestElement() {
            ...
            for (StarNode i : nodes) {
                if (i.estimatedTotalCost < best.estimatedTotalCost) {
                    best = i;
                }
            }
            return best;
