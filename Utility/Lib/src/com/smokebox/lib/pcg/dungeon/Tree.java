...
public class Tree {
	...
	public Tree(ArrayList<Cell> cells) {
		...
		while(openNodes.size() > 0) { // While there are nodes that are not in the tree
			...
			for(int i = 0; i < openNodes.size(); i++) {
				if(openNodes.get(i).id == bestOpenNode) {
					...
					openNodes.remove(i);
					System.out.println("Nodes left in openList: " + openNodes.size());
				}
			}
		}
		
		Collections.sort(nodesInTree, new Node.OrderById());
